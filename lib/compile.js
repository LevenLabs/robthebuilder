var path = require('path'),
    ...
    outDir = path.join(currentDir, 'out'),
    headerRegex = /^<!--\s*(\{[\s\S]*\})\s*-->([\s\S]*)/m, //by default . does not match new-lines but \s does, and [^] matches anything
    minifyOptions = {
        removeComments: true,
...
function singlePassProcessTemplates(templates) {
    ...
    for (name in templates) {
        ...
        if (file.header.parent) {
            ...
            if (parent.args) {
                params = _assign(params, parent.args);
            }
            if (parent.include_var) {
                params[parent.include_var] = file.content;
            }
            file.content = templates[parent.name].templateize(params)(params);
            file.templateize = (function(pn, p, incv) {
                return function(overrideParams) {
                    var newParams = _assign({}, p, overrideParams),
                        content = '';
                    // don't allow them to override the include_var's params
                    if (incv) {
                        newParams[incv] = p[incv];
                    }
                    content = templates[pn].templateize(newParams)(newParams);
                    return _template(minify(content, minifyOptions));
                };
            }(parent.name, params, parent.include_var));
        } else {
            (function(f) {
                f.templateize = function() {
                    return _template(minify(f.content, minifyOptions));
                };
            }(file));
        }
